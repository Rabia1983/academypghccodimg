		...
		public static void Main(string[] args)
		{
			...
			while (keepplaying == true)
			{
				...
				if (answer == "Q")
				{
					...
					reader.Close();
					SqliteCommand resultcommand = new SqliteCommand($"select results.id, Count(*) AS Total from results inner join Answers on results.id=answers.result_id inner join UserAnswers on answers.id = useranswers.answer_id where useranswers.user_id ={userid} GROUP BY results.id ORDER BY Total DESC limit 1;", connection);
					reader = resultcommand.ExecuteReader();
					reader.Read();
					string result_id = reader["id"].ToString();
					reader.Close();
					command = new SqliteCommand($"SELECT * FROM Results WHERE id={result_id}", connection);
					reader = command.ExecuteReader();
